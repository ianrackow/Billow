...
var Gmail = function (localJQuery) {
  ...
  if (window_opener) {
    try {
      ...
      if (opener_domain !== window.document.domain) {
        console.warn(
          "GmailJS: window.opener domain differs from window domain."
        );
        window_opener = null;
      }
    } catch (error) {
  ...
  function oldGmailApiDeprecated(
    text = "Migrate to new API compatible with new Gmail to silence this warning!"
  ) {
  ...
  api.get.delegated_to_email = function () {
    ...
    const delegatedToUserIndex = parseInt(
      pathname.substring(
        pathname.indexOf(userIndexPrefix) + userIndexPrefix.length
      ),
      10
    );

    const loggedInAccounts = api.get.loggedin_accounts();
    const loggedInAccount = loggedInAccounts.find(
      (account) => account.index === delegatedToUserIndex
    );
  ...
  api.helper.get.is_locale = function (locale) {
    ...
    return (
      localePrefix.toLowerCase() === localePrefix ||
  ...
  api.check.should_compose_fullscreen = function () {
    console.warn(
      "gmail.js: This function is known to be unreliable, and may be deprecated in a future release."
    );
    var bx_scfs = [];
    try {
  ...
  api.check.is_streak_installed = function () {
    return (
      $(
        "[id^='bentoBox'],[id*=' bentoBox'],[class*=' bentoBox'],[class*='bentoBox']"
      ).length > 0
    );
  };
  ...
  api.check.is_anydo_installed = function () {
    return (
      $("[id^='anydo'],[id*=' anydo'],[class*=' anydo'],[class*='anydo']")
        .length > 0
    );
  };
  ...
  api.check.is_boomerang_installed = function () {
    return (
      $("[id^='b4g_'],[id*=' b4g_'],[class*=' b4g_'],[class*='b4g_']").length >
      0
    );
  };
  ...
  api.check.is_signal_installed = function () {
    return (
      $("[id^='Signal'],[id*=' Signal'],[class*=' signal'],[class*='signal']")
        .length > 0
    );
  };
    ...
    if (dom || dom.length > 0) {
      ...
      if (dom[0].title.indexOf(title) !== -1) {
        const value = parseInt(
          dom[0].attributes["aria-label"].value.replace(/[^0-9]/g, "")
        );
        if (!isNaN(value)) {
          return value;
        }
  ...
  api.tools.parse_actions = function (params, xhr) {
    ...
    if (
      params.url.act === "fup" ||
      params.url.act === "fuv" ||
      params.body_is_object
    ) {
      return params.body_is_object && api.observe.bound("upload_attachment")
        ? { upload_attachment: [params.body_params] }
        : false; // trigger attachment event
    }
    ...
    var email_ids =
      typeof sent_params.t === "string" ? [sent_params.t] : sent_params.t;
    var response = null;
    ...
    if (
      typeof params.url._reqid === "string" &&
      params.url.view === "tl" &&
      params.url.auto !== undefined
    ) {
    ...
    if (
      typeof params.url.SID === "string" &&
      typeof params.url.zx === "string" &&
      params.body.indexOf("req0_") !== -1
    ) {
    ...
    if (
      response &&
      action_map[action] &&
      api.observe.bound(action_map[action])
    ) {
  ...
  api.check.data.is_action = function (obj) {
    return (
      api.check.data.is_first_type_action(obj) ||
  ...
  api.tools.get_action = function (obj) {
    return (
      api.tools.get_first_type_action(obj) ||
  ...
  api.tools.check_event_type = function (threadObj) {
    ...
    if (threadData && api.check.data.is_action(threadData)) {
      ...
      if (
        action.startsWith(apply_label) &&
        api.check.data.is_first_type_action(threadData)
      ) {
    ...
    try {
      ...
      for (let fd_html_container of fd_html_containers) {
        fd_email_content_html =
          (fd_email_content_html || "") + fd_html_container["2"]["1"];
      }
    } catch (e) {
  ...
  api.tools.parse_fd_request_payload_get_email2 = function (
    fd_thread_container,
    fd_email_id
  ) {
    try {
  ...
  api.tools.parse_fd_embedded_json_get_email = function (
    fd_thread_container,
    fd_email_id
  ) {
    try {
    ...
    try {
      ...
      for (let fd_thread_container of fd_threads) {
        ...
        for (let fd_email of fd_emails) {
          ...
          const fd_email2 = api.tools.parse_fd_request_payload_get_email2(
            fd_thread_container,
            fd_email_id
          );
          ...
          const fd_email_is_draft = api.tools.parse_fd_bv_is_draft(
            fd_email2["3"]
          );

          const fd_email_content_html =
            api.tools.parse_fd_request_html_payload(fd_email);

          const fd_attachments = api.tools.parse_fd_attachments(
            fd_email["1"]["13"]
          );
      ...
      console.warn(
        "Gmail.js encountered an error trying to parse email-data on fd request!",
        error
      );
      return null;
    }
    ...
    try {
      ...
      for (let fd_thread_container of fd_threads) {
        ...
        for (let fd_email of fd_emails) {
          ...
          const fd_email2 = api.tools.parse_fd_embedded_json_get_email(
            fd_thread_container,
            fd_email_id
          );
          ...
          //TODO : need a refactoring
          const fd_email_content_html =
            api.tools.parse_fd_embedded_json_content_html(fd_email);

          const fd_attachments = api.tools.parse_fd_embedded_json_attachments(
            fd_email["11"]
          );
      ...
      console.warn(
        "Gmail.js encountered an error trying to parse email-data on embedded json!",
        error
      );
      return null;
    }
    ...
    try {
      ...
      for (let bv_thread_container of bv_threads) {
        ...
        for (let bv_email of bv_emails) {
          ...
          const bv_email_is_draft = api.tools.parse_fd_bv_is_draft(
            bv_email["10"]
          );
      ...
      console.warn(
        "Gmail.js encountered an error trying to parse email-data on bv request!",
        error
      );
      return null;
    }
  ...
  api.tools.parse_sent_message_html_payload = function (sent_email) {
    ...
    try {
      ...
      for (let sent_html_container of sent_html_containers) {
        sent_email_content_html =
          (sent_email_content_html || "") + sent_html_container["2"];
      }
    } catch (e) {
      // don't crash gmail when we cant parse email-contents
    }
  ...
  api.tools.parse_sent_message_payload = function (json) {
    try {
      ...
      const sent_email_content_html =
        api.tools.parse_sent_message_html_payload(sent_email);
      const sent_email_ishtml = sent_email["8"]["6"];

      const sent_attachments = api.tools.parse_sent_message_attachments(
        sent_email["11"]
      );
      ...
      console.warn(
        "Gmail.js encountered an error trying to parse sent message!",
        error
      );
      return null;
    }
    ...
    try {
      ...
      const parse_sent_message_html_payload_new = (sent_email) => {
        ...
        try {
          ...
          for (let sent_html_container of sent_html_containers) {
            sent_email_content_html =
              (sent_email_content_html || "") + sent_html_container[1];
          }
        } catch (err) {
          // don't crash gmail when we cant parse email-contents
      ...
      const sent_email_content_html =
        parse_sent_message_html_payload_new(sent_email);
      const sent_email_ishtml = sent_email[8][6];
      const sent_attachments = parse_sent_message_attachments_new(
        sent_email[11]
      );
  ...
  api.tools.parse_request_payload = function (params, events, force) {
    ...
    const threads = api.tools.extract_from_graph(
      params,
      api.check.data.is_thread
    );
    // console.log("Threads:");
    // console.log(threads);
    const emails = [
      ...api.tools.extract_from_graph(
        params.body_params,
        api.check.data.is_email
      ),
      ...api.tools.extract_from_graph(
        params.body_params,
        api.check.data.is_email_new
      ),
    ];
      ...
      if (Array.isArray(threads) && api.check.data.is_thread(threads[0])) {
        ...
        if (actionType) {
          ...
          const threadsData = threads.map((thread) =>
            api.tools.get_thread_data(thread)
          );

          const new_thread_ids = threads.map((thread) =>
            api.tools.get_thread_id(thread)
          );
          const new_email_ids = threadsData
            .map((threadData) => api.tools.get_message_ids(threadData))
  ...
  api.tools.parse_requests = function (params, xhr) {
    ...
    if (
      typeof api.tracker.events !== "object" &&
      typeof api.tracker.actions !== "object"
    ) {
      api.tracker.events = [];
      api.tracker.actions = [];
    }
  ...
  api.tools.cache_email_data = function (email_data, data_source) {
    ...
    if (
      data_source === "fd_request_payload" ||
      data_source === "fd_embedded_json"
    ) {
  ...
  api.tools.xhr_watcher = function () {
    ...
    api.instanceId = Symbol(
      "gmail-js-" + (performance ? performance.now() : Date.now())
    );
    ...
    api.tools.patch(win.XMLHttpRequest.prototype.send, (orig) => {
      win.XMLHttpRequest.prototype.send = function (body) {
        ...
        if (
          api.observe.bound(events, "after") ||
          api.check.is_new_data_layer()
        ) {
          ...
          this.onreadystatechange = function (progress) {
            if (this.readyState === this.DONE) {
              if (
                progress.target.responseType === "" ||
                progress.target.responseType === "text"
              ) {
                xhr.xhrResponse = api.tools.parse_response(
                  progress.target.responseText
                );
              } else {
              ...
              if (api.check.is_new_data_layer()) {
                const pathName = api.tools.get_pathname_from_url(
                  xhr.xhrParams.url_raw
                );
                if (pathName.endsWith("/i/fd")) {
                  let parsed_emails = api.tools.parse_fd_request_payload(
                    xhr.xhrResponse
                  );
                  if (parsed_emails !== undefined && parsed_emails !== null) {
                    api.tools.cache_email_data(
                      parsed_emails,
                      "fd_request_payload"
                    );
                ...
                if (pathName.endsWith("/i/bv")) {
                  let parsed_emails = api.tools.parse_bv_request_payload(
                    xhr.xhrResponse
                  );
                  if (parsed_emails !== undefined && parsed_emails !== null) {
                    api.tools.cache_email_data(
  ...
  api.tools.embedded_data_watcher = function () {
    ...
    window._GM_setData = function (data) {
      if (
        data !== undefined &&
        data.Cl6csf !== undefined &&
        data.Cl6csf[0] !== undefined &&
        data.Cl6csf[0][2] !== undefined
      ) {
        //console.log('Cl6csf',JSON.parse(data.Cl6csf[0][2]));
        let parsed_emails = api.tools.parse_fd_embedded_json(
          JSON.parse(data.Cl6csf[0][2])
        );
      ...
      if (
        data !== undefined &&
        data.a6jdv !== undefined &&
        data.a6jdv[0] !== undefined &&
        data.a6jdv[0][2] !== undefined
      ) {
        //console.log('a6jdv',JSON.parse(data.a6jdv[0][2]));
        let parsed_emails = api.tools.parse_bv_embedded_json(
          JSON.parse(data.a6jdv[0][2])
        );
  ...
  api.observe.bind = function (type, action, callback) {
    ...
    api.tracker.bound[action] =
      typeof api.tracker.bound[action] === "undefined"
        ? 1
        : api.tracker.bound[action] + 1;
    api.tracker.bound[type] =
      typeof api.tracker.bound[type] === "undefined"
        ? 1
        : api.tracker.bound[type] + 1;
  ...
  api.observe.off = function (action, type) {
    ...
    $.each(types, function (idx, type) {
      ...
      if (action) {
        if (typeof api.tracker.watchdog[type][action] === "object") {
          api.tracker.bound[action] -=
            api.tracker.watchdog[type][action].length;
  ...
  api.observe.initialize_dom_observers = function () {
    ...
    api.tracker.supported_observers = [
      "view_thread",
      "view_email",
      "load_email_menu",
      "recipient_change",
    ...
    if (api.tracker.custom_supported_observers) {
      ...
      ); // deep copy to copy in sub_observers where relevant
    }
  ...
  api.observe.register = function (action, args) {
    ...
    if (typeof args === "object" && !$.isArray(args)) {
      ...
      $.each(
        ["class", "selector", "sub_selector", "handler"],
        function (idx, arg) {
          if (args[arg]) {
            config[arg] = args[arg];
          }
        ...
        var mutationObserver = new MutationObserver(function (mutations) {
          for (var i = 0; i < mutations.length; i++) {
            ...
            for (var j = 0; j < removedNodes.length; j++) {
              ...
              if (
                removedNode.className === "agh" &&
                removedNode.querySelector("div[data-hovercard-id]")
              ) {
                // contains recipient in peoplekit
                let observer = api.tracker.dom_observer_map["afV"];
                let handler =
                  api.tracker.dom_observers.recipient_change.handler;
  ...
  api.tools.insertion_observer = function (
    target,
    dom_observers,
    dom_observer_map,
    sub
    ...
    $.each(classes, function (idx, className) {
      ...
      for (var observer of observers) {
        ...
        if (
          observer &&
          api.tracker.watchdog &&
          api.tracker.watchdog.dom[observer]
        ) {
          var element = $(target);
  ...
  api.tools.make_request_async = function (
    _link,
    method,
    callback,
    error_callback,
  ...
  api.helper.get.visible_emails_pre = function (customInboxQuery) {
    ...
    if (page.indexOf("label/") === 0) {
      ...
      if ($("div[aria-label='Social']").attr("aria-selected") === "true") {
        ...
        $("div[aria-label='Promotions']").attr("aria-selected") === "true"
      ) {
        cat_label = "promo";
        url += "&cat=^smartlabel_" + cat_label + "&search=category";
      } else if (
        $("div[aria-label='Updates']").attr("aria-selected") === "true"
      ) {
        cat_label = "notification";
        url += "&cat=^smartlabel_" + cat_label + "&search=category";
      } else if (
        $("div[aria-label='Forums']").attr("aria-selected") === "true"
      ) {
        cat_label = "group";
        url += "&cat=^smartlabel_" + cat_label + "&search=category";
      } else {
  ...
  api.tools.infobox = function (message, time, html) {
    ...
    if (top.length > 0) {
      ...
      if (typeof time !== "undefined") {
        ...
        top.removeAttr("style").fadeTo(time, 0, function () {
          // simply remove then restore
  ...
  api.tools.parse_email_data = function (email_data) {
    ...
    for (var i in email_data) {
      ...
      if (x[0] === "ms") {
        ...
        data.threads[x[1]].attachments_details = x[13]
          ? api.tools.parse_attachment_data(x[13])
          : null;
        data.threads[x[1]].subject = x[12];
        data.threads[x[1]].content_html = x[13] ? x[13][6] : x[8];
        data.threads[x[1]].to = x[13]
          ? x[13][1]
          : x[37] !== undefined
          ? x[37][1]
          : [];
        ...
        try {
          // jQuery will sometime fail to parse x[13][6], if so, putting the raw HTML
  ...
  api.helper.get.email_data_pre = function (thread_id) {
    oldGmailApiDeprecated(
      "Migrate code to use gmail.new.get.email_data() to fix this problem."
    );
  ...
  api.helper.get.new_email_id = function (identifier) {
    if (!identifier) {
      ...
      // ensure to only email_data, not DomEmail!
  ...
  api.get.email_source_async = function (
    identifier,
    callback,
    error_callback,
    preferBinary
    ...
    api.get
      .email_source_promise(identifier, preferBinary)
      .then(callback)
  ...
  api.get.displayed_email_data = function () {
    ...
    if (api.check.is_conversation_view()) {
      ...
      // Supposing only one displayed email.
  ...
  var get_displayed_email_data_for_single_email = function (email_data) {
    ...
    for (var id in email_data.threads) {
      var displayed_email_element = document.querySelector(
        "div[data-legacy-message-id='" + id + "']"
      );
      ...
      if (displayed_email_element) {
        ...
        email.to.forEach(function (recipient) {
          ...
          var name =
            api.tools.extract_name(recipient.replace(address, "")) || "";

          displayed_email_data.people_involved.push([name, address]);
        });
  ...
  var create_generic_toolbar_button = function (
    content_html,
    onClickFunction,
    basicStyle,
    defaultStyle,
  ...
  api.tools.add_toolbar_button = function (
    content_html,
    onClickFunction,
    styleClass
  ) {
    ...
    return create_generic_toolbar_button(
      content_html,
      onClickFunction,
      basicLeftStyle,
      defaultLeftStyle,
  ...
  api.tools.add_right_toolbar_button = function (
    content_html,
    onClickFunction,
    styleClass
  ) {
  ...
  api.tools.add_compose_button = function (
    composeWindow,
    content_html,
    onClickFunction,
    styleClass
    ...
    div.attr(
      "style",
      "cursor: pointer !important; transform: translateY(1px);"
    );

    var button = $(document.createElement("div"));
    var buttonClasses =
      "T-I J-J5-Ji aoO T-I-atl L3 gmailjs gmailjscomposebutton ";
  ...
  api.tools.add_more_send_option = function (
    composeWindow,
    buttonText,
    onClickFunction,
    styleClass,
  ...
  /**
       ...
       */
  api.tools.add_attachment_button = function (
    attachment,
    contentHtml,
    customCssClass,
    tooltip,
  ...
  api.tools.add_modal_window = function (
    title,
    content_html,
    onClickOk,
    onClickCancel,
  ...
  api.dom.visible_messages = function () {
    ...
    $('tbody>tr.zA[role="row"]:visible', api.dom.inbox_content()).each(
      (index, msgEle) => {
        ...
        ret.push({
          ...
          thread_id: api.helper.clean_thread_id(
            (idNode && idNode.dataset && idNode.dataset.threadId) || ""
          ),
          legacy_email_id:
            (idNode && idNode.dataset && idNode.dataset.legacyMessageId) || "",
          $el: $(msgEle),
        });
      }
    );
  ...
  extend(api.dom.compose.prototype, {
    ...
    recipients: function (options) {
      ...
      const found = peopleKit
        ? this.$el
            .find(
              "tr.bzf " +
                (type_selector || "div[name]") +
                " div[data-hovercard-id]"
            )
            .map((_, el) => ({
              type:
                options.type || el.closest("div[name]").getAttribute("name"),
              email: el.getAttribute("data-hovercard-id"),
            }))
        : this.$el
            .find(".GS input[type=hidden]" + type_selector)
            .map((_, el) => ({
              type: el.name,
              email: el.value,
            }));
    ...
    dom: function (lookup) {
      ...
      if (!config[lookup])
        api.tools.error(
          'Dom lookup failed. Unable to find config for "' + lookup + '"',
          config,
          lookup,
          config[lookup]
        );
  ...
  api.dom.email = function (element) {
    ...
    if (
      ...
      api.check.data.is_legacy_email_id(element)
    ) {
      this.id = element;
      this.$el = $("div.adn[data-legacy-message-id='" + this.id + "']");
    } else if (
      typeof element === "string" &&
      api.check.data.is_email_id(element)
    ) {
      const elem = document.querySelector(
        "div.adn[data-message-id='" +
          element.replace("msg-f:", "\\#msg-f\\:") +
          "']"
      );
      ...
      ((element.classList && element.classList.contains("adn")) || // DOM
        (element.hasClass && element.hasClass("adn")))
    ) {
      // jQuery
      this.$el = $(element);
